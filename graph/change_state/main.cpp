...
int main(int argc, char *argv[])
{
    ...
    pStateOff->assignProperty(plblOff, "visiЬle", true);
    ...
    pStateOn->assignProperty(plblOff, "visiЬle", false);
    pStateOn->assignProperty (plblOn, "visiЬle", true);
    QSignalTransition* ptransl = pStateOff->addTransition(pcmd, SIGNAL(clicked()), pStateOn);
    QSignalTransition* ptrans2 = pStateOn->addTransition(pcmd, SIGNAL(clicked()), pStateOff);

    QPropertyAnimation* paniml = new QPropertyAnimation(pcmd, "geometry");
